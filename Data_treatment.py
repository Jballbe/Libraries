...
def create_cell_file(cell_id_list, saving_folder_path="/Volumes/Work_Julien/Cell_Data_File/", selection=['All'], overwrite=False):
    ...
    for cell_id in tqdm(cell_id_list):
        ...
        if cell_id not in Cell_file_information['Cell_id']:
            ...
            empty_array[:] = np.nan
            new_cell_id_line = pd.Series([cell_id,*empty_array],index=Cell_file_information.columns)
            Cell_file_information=Cell_file_information.append(new_cell_id_line,ignore_index=True)
...
def compute_cell_features(cell_id,Full_SF_table,cell_sweep_info_table,time_response_list):
    ...
    I_O_pre_obs='--'
    # if max_freq_step_norm >= .5:
    #     I_O_pre_obs='max_freq_step_norm_higher_than_0.4'
    
   


    ## If I/O criteria passed, try to compute adaptation
    ...
    if adapt_do_fit == False or Adaptation_obs!='--':
        ...
        new_fit_table_line = pd.Series([0,
                                        I_O_pre_obs,
                                        np.nan,
                                        np.nan,
                                        np.nan,
        ...
        new_feature_table_line = pd.Series([0,
                                            np.nan,
                                            np.nan,
                                            np.nan,
                                            np.nan,
                                            np.nan], index=feature_columns)
    ...
    for response_time in time_response_list:
        ...
        if do_fit == True:
            I_O_obs, Hill_Amplitude, Hill_coef, Hill_Half_cst,Hill_x0, sigmoid_x0,sigmoid_sigma,I_O_QNRMSE, x_shift, Gain, Threshold, Saturation_freq,Saturation_stim = fitlib.fit_IO_curve(
    ...
    if database == 'Allen_Cell_Type_Database':
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
            TPC_lines_dict = {executor.submit(
                get_sweep_TPC, x, cell_id, database, my_Cell_data): x for x in coarse_sweep_number}
            for f in concurrent.futures.as_completed(TPC_lines_dict):
                Full_TPC_table = Full_TPC_table.append(
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
            TPC_lines_dict = {executor.submit(
                get_sweep_TPC, x, cell_id, database, '--', stim_start_time, stim_end_time): x for x in sweep_list}
            for f in concurrent.futures.as_completed(TPC_lines_dict):
                Full_TPC_table = Full_TPC_table.append(
                    f.result(), ignore_index=True)
        ...
        cell_sweep_table = pd.read_pickle(
            '/Volumes/Work_Julien/NVC_database/NVC_CC_Cell_sweep_table.pkl')
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
            TPC_lines_dict = {executor.submit(
                get_sweep_TPC, x, cell_id, database, '--'): x for x in sweep_list}
            for f in concurrent.futures.as_completed(TPC_lines_dict):
                Full_TPC_table = Full_TPC_table.append(
                    f.result(), ignore_index=True)
...
def get_sweep_TPC(current_sweep, cell_id, database, my_Cell_data, stim_start_time=None, stim_end_time=None):
    '''
    ...
    if database == 'Allen_Cell_Type_Database':
        ...
        TPC_new_line = pd.Series(
            [str(current_sweep), original_TPC], index=['Sweep', 'TPC'])

    elif database == 'Lantyer_Database':

        current_time_array, current_membrane_trace, current_stim_array = get_traces(
        ...
        current_time_array, current_membrane_trace, current_stim_array,stim_start_time, stim_end_time = get_traces(
...
def get_traces(cell_id, sweep_id, database):
    ...
    if database == 'Allen_Cell_Type_Database':
        ...
        # cell, sweep, stim_amp, stim_start_time, stim_end_time = sweep_stim_table[
        #     sweep_stim_table['Sweep_id'] == sweep_id].iloc[0, :]
        
        sweep_list=sweep_stim_table.iloc[0,2]
...
def create_cell_sweep_info_table_concurrent_runs(cell_id, database, cell_Full_TPC_table):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        sweep_lines_dict = {executor.submit(get_sweep_info, cell_Full_TPC_table, x,
                                            cell_id, database, stim_start_time, stim_end_time): x for x in sweep_list}

        for f in concurrent.futures.as_completed(sweep_lines_dict):
            cell_sweep_info_table = cell_sweep_info_table.append(
                f.result(), ignore_index=True)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        sweep_linear_dict = {executor.submit(ephys_treat.get_sweep_linear_properties_new, x,cell_Full_TPC_table, cell_sweep_info_table): x for x in sweep_list}
    ...
    for current_Protocol in cell_sweep_info_table['Protocol_id'].unique():
        ...
        extrapolated_BE = extrapolated_BE.append(
            extrapolated_BE_Series, ignore_index=True)
...
def get_sweep_info(cell_Full_TPC_table, current_sweep, cell_id, database, stim_start_time, stim_end_time):
    ...
    if database == 'Allen_Cell_Type_Database':
        ...
        current_TPC=get_filtered_TPC_table(cell_Full_TPC_table,current_sweep,do_filter=True,filter=5.,do_plot=False)
    ...
    output_line = pd.Series([str(current_sweep),
                             Protocol_id,
                             trace_id,
                             stim_amp,
                             stim_baseline,
                             ...
                             Bridge_Error,
                             BE_extrapolated,
                             sampling_rate], index=['Sweep', 'Protocol_id', 'Trace_id', 'Stim_amp_pA', 'Stim_baseline_pA', 'Stim_start_s', 'Stim_end_s', 'Bridge_Error_GOhms', 'Bridge_Error_extrapolated', 'Sampling_Rate_Hz'])
...
def create_cell_Full_SF_dict_table(original_Full_TPC_table, original_cell_sweep_info_table):
    ...
    for current_sweep in sweep_list:
        ...
        new_line = pd.Series([str(current_sweep), SF_dict],
                             index=['Sweep', 'SF_dict'])
        Full_SF_dict_table = Full_SF_dict_table.append(
            new_line, ignore_index=True)
...
def create_Full_SF_table(original_Full_TPC_table, original_Full_SF_dict):
    ...
    for current_sweep in sweep_list:
        ...
        new_line = pd.Series(
            [str(current_sweep), current_SF_table], index=["Sweep", "SF"])
...
def create_cell_sweep_QC_table(cell_sweep_info_table):
    ...
    for sweep in sweep_id_list:
        ...
        sweep_line = pd.Series([str(sweep), pass_sweep_QC],
                               index=["Sweep", 'Passed_QC'])
        sweep_QC_table = sweep_QC_table.append(sweep_line, ignore_index=True)
...
def import_h5_file(file_path, selection=['All']):
    ...
    if 'TPC_SF' in selection and 'TPC_tables' in current_file.keys():
        ...
        with Pool() as pool:
            ...
            for result in pool.starmap(get_TPC_SF_table_concurrent_runs_bis, params, chunksize=(1)):
                Full_TPC_table = Full_TPC_table.append(
...
def estimate_cell_stim_time_limit_concurrent_runs(cell_id, database):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        sweep_lines_dict = {executor.submit(
            estimate_sweep_stim_time_limit, cell_id, x, database): x for x in sweep_list}
        for f in concurrent.futures.as_completed(sweep_lines_dict):
...
def estimate_sweep_stim_time_limit(cell_id, sweep, database):
    '''
    ...
    stim_table['Input_current_pA']=np.array(filter_trace(stim_table['Input_current_pA'],stim_table['Time_s'],filter=5.,do_plot=False))  
    ...
    stim_table = get_autocorrelation(stim_table, .500, do_plot=False)
...
def get_min_freq_step(original_stimulus_frequency_table,do_plot=False):
    ...
    minimum_threshold = maximum_fit_frequency/10
    
    
   
    
    until_maximum_data = original_data_table[original_data_table['Stim_amp_pA']<=maximum_fit_stimulus].copy()
    until_maximum_data = until_maximum_data[until_maximum_data['Frequency_Hz']>=minimum_threshold]
    ...
    if do_plot:
        ...
        minimum_freq_step_plot += geom_hline(yintercept = minimum_threshold,linetype='dashed',alpha=.4)
        
        print(minimum_freq_step_plot)
        
    return minimum_freq_step,minimum_freq_step_stim,minimum_threshold,np.nanmax(original_data_table['Frequency_Hz'])
